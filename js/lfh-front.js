    ...
    lfh.Map = function (i) {
        ...
        function _add_markers() {
            ...
            for (var i in _data.markers) {
                if (typeof _data.markers[i] != 'function') {
                    _add_marker(i);
                }
            }
        }
        function _add_gpx_polylines() {
            for (var j in _data.gpx) {
                if (typeof _data.gpx[i] != 'function') {
                    _add_gpx(j);
                }
            }
        }
        ...
        function _add_marker(i) {
            ...
            _markers[i] = L.marker([info.lat, info.lng], {
                elem_id: marker_id,
        ...
        function _add_loaded_listener(buttonreset) {
            ...
            var isLoaded = 1;
            for (var j in _data.gpx) {
                if (typeof _data.gpx[j] != 'function') {
        ...
        function _add_gpx(j) {
            ...
            _gpx[j] = new L.GPX(_data.gpx[j].src, {
                async: true,
                isLoaded: false,
                elem_id: track_id,
                marker_options: {
                ...
                var link = new lfh.Link(map, e.target, track_id, _selected_element, _move_marker, _data.gpx[j].unit, _data.gpx[j].unit_h);
    ...
    lfh.Link = function (map, layer, elem_id, selected, move, unit, unit_h) {
        ...
        function _initialize() {
            ...
            if (_dom != null) {
                ...
                _add_event();
                if (_layer instanceof L.GPX) {
                    var profile = new lfh.Profile(_map, _layer, _dom, _move_marker, _unit, _unit_h);
                }
            }
        }
