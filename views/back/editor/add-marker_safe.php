...
if(!isset($plugin_url)){
    ...
    $colors = Lfh_Tools_Map::$colors_marker;
    $icons  = Lfh_Tools_Map::$icons_marker;
    $class = Lfh_Tools_Map::$class_map;
    $default = Lfh_Tools_Map::$default;
    $tiles = Lfh_Tools_Map::$tiles;
...
if(!function_exists('__')){
    function __($text, $domaine){
        return stripslashes($text);
    }
    $options_map = Lfh_Tools_Map::map_parameters();
}
//-- end for dev
...
include $plugin_dir.'/views/back/editor/map-form.phtml';?>
...
var lfh = {
        ...
        set_tile: function(){
            var tileinfo = lfh.tiles[document.querySelector('select[name="map-tile"]').value];
            if(lfh.tile != null){
        ...
        shortcode_map:function(){
            ...
            shortcode += ' autoCenter=' + document.querySelector('input[name="autocenter"]').checked;
            if(!document.querySelector('input[name="autocenter"]').checked){
...
// event on elements of window edit marker and edit map
   ...
   // - for edit map
   //-----------------
   var node = document.querySelector('#window-edit-map input[name="autocenter"]');
   L.DomEvent.addListener(node, 'click', function(e){
